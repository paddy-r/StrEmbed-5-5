...
There are several ways to run ```StrEmbed-5-5```, which are described below and in more detail in the user manual. The simplest is to download the latest release, which contains all the necessary scripts and standalone executable files. To run ```StrEmbed-5-5``` you can then do one of the following:
